...
/// {@template first_test_api_malformed_response}
/// An exception thrown when there is a problem decoded the response body.
/// {@endtemplate}
class FirstTestApiMalformedResponse implements Exception {
  /// {@macro first_test_api_malformed_response}
  const FirstTestApiMalformedResponse({required this.error});

  /// The associated error.
  final Object error;
}

/// {@template first_test_api_request_failure}
/// An exception thrown when an http request failure occurs.
/// {@endtemplate}
class FirstTestApiRequestFailure implements Exception {
  /// {@macro first_test_api_request_failure}
  const FirstTestApiRequestFailure({
...
/// {@template first_test_api_client}
/// A Dart API client for the firstTest API.
/// {@endtemplate}
class FirstTestApiClient {
  /// Create an instance of [FirstTestApiClient] that integrates
  /// with the remote API.
  ///
  /// {@macro first_test_api_client}
  FirstTestApiClient({
  ...
  ///
  /// {@macro first_test_api_client}
  FirstTestApiClient.localhost({
    required TokenProvider tokenProvider,
    http.Client? httpClient,
  }) : this._(
          // baseUrl: 'http://10.0.2.2:8080',
          baseUrl: 'http://192.168.0.236:80', //change to your local ip
          httpClient: httpClient,
          tokenProvider: tokenProvider,
        );

  /// {@macro first_test_api_client}
  FirstTestApiClient._({
  ...
  Future<ArticleResponse> getArticle({
    ...
    if (response.statusCode != HttpStatus.ok) {
      throw FirstTestApiRequestFailure(
        body: body,
  ...
  Map<String, dynamic> json() {
    try {
      ...
      Error.throwWithStackTrace(
        FirstTestApiMalformedResponse(error: error),
        stackTrace,
      );
    }
  }
